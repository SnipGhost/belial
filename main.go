...
func decryptCycleCode(code, n, k uint64) (uint64, uint64, error) {
	...
	if code >= (1 << n) { // Проверка на то, что степень принятого полинома меньше n
		return 0, 0, fmt.Errorf("Code is too big")
	}
...
func encryptWrapper(dataStr string, codeLen, infLen uint64, codeType byte) (uint64, error) {
	if codeType != 0 {
...
func decryptWrapper(code, codeLen, infLen uint64, codeType byte) (uint64, uint64, error) {
	if codeType != 0 {
		return 0, 0, fmt.Errorf("Not implemented")
	}
	res, rem, err := decryptCycleCode(code, codeLen, infLen)
	if err != nil {
		return 0, 0, fmt.Errorf("Can't encrypt data: %s", err)
	}
	// if rem != 0 {
...
func check(code string, n, k uint64, codeType byte) {
	encrypted, err := encryptWrapper(code, n, k, codeType)
	if err != nil {
		fmt.Println("Error with encrypting:", err)
		return
	}
	var e, rem, i uint64
	var val float64
	var limit uint64 = (1 << (n + 1)) - 1
	success := make([]uint64, n+1)
	for e = 0; e <= limit; e++ {
		_, rem, err = decryptWrapper(encrypted^e, n, k, codeType)
		if err != nil {
			fmt.Println("Error with decrypting:", err)
			return
		}
	...
	for i = 0; i < n; i++ {
		val = float64(success[i]) / float64(countCombinations(n, i)) * 100.0
		fmt.Printf("Test for %d bits in err vector: %3.2f\n", i, val)
	}
}

func main() {
	check("01011010001", 15, 11, 0)
